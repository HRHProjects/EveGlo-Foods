...
const experienceAssets = {
  carton: '/assets/eveglo/eveglo-product-in-box.jpg',
  polished: '/assets/eveglo/fresh-and-natural-rice-indulgence.png',
  front: '/assets/eveglo/eveglo-front.jpg',
  back: '/assets/eveglo/eveglo-back.jpg',
  blueprint: '/assets/eveglo/eveglo-flat-carton.jpg'
};

const languageModes = {
  english: {
    label: 'English',
    headline: 'Instant Shirataki Rice for lighter everyday meals.',
    subcopy: 'A rice-style, plant-based pantry option with 35 calories and 7g carbohydrate per 50g serving.',
    prompt: 'Build a bowl. Stir into soup. Serve with everyday seasonings.'
  },
  french: {
    label: 'French',
    headline: 'Riz shirataki instantane pour des repas plus legers au quotidien.',
    subcopy: 'Une option de garde-manger a texture de riz, avec 35 calories et 7 g de glucides par portion de 50 g.',
    prompt: 'Preparez un bol, ajoutez-le a une soupe ou servez-le avec vos assaisonnements favoris.'
  },
  bilingual: {
    label: 'Bilingual',
    headline: 'Instant Shirataki Rice / Riz shirataki instantane',
    subcopy: 'A clean, rice-style pantry option designed for Canadian shelves and everyday meals.',
    prompt: 'Taste The Glow of Nature / Goutez l eclat de la nature'
  }
};

const packageHotspots = [
  {
    id: 'calories',
    label: '35 Calories',
    x: 54,
    y: 72,
    title: '35 calories per serving',
    body: 'A light rice-style option for bowls, soups, stir-fries, risotto-style plates, and everyday meals.'
  },
  {
    id: 'carbs',
    label: '7g Carbohydrate',
    x: 54,
    y: 65,
    title: '7g carbohydrate per 50g serving',
    body: 'Clear front-of-pack nutrition communication helps shoppers understand the product quickly.'
  },
  {
    id: 'bilingual',
    label: 'Bilingual package',
    x: 52,
    y: 25,
    title: 'English and French product naming',
    body: 'The front panel is built for a Canadian retail environment with bilingual product communication.'
  },
  {
    id: 'icons',
    label: 'Trust icons',
    x: 80,
    y: 88,
    title: 'Vegan, halal, and GMO-free cues',
    body: 'Fast-recognition trust signals support quick shelf decisions without overcrowding the package.'
  },
  {
    id: 'texture',
    label: 'Rice texture',
    x: 50,
    y: 47,
    title: 'Rice-style texture cue',
    body: 'The central product visual helps shoppers connect shirataki rice with familiar meal use cases.'
  }
];
...
const blueprintPanels = [
  {
    ...
    x: 58,
    y: 25,
    note: 'Main shopper-facing story: product name, rice visual, nutrition callouts, and dietary icons.'
  },
  {
    id: 'barcodeArea',
    label: 'Barcode area',
    x: 42,
    y: 78,
    note: 'Retail-use placement that supports scanning and shelf operations.'
  },
  {
    id: 'sideInstructions',
    label: 'Side panel',
    x: 14,
    y: 33,
    note: 'Compact preparation and product information area for side-shelf review.'
  }
];

const ingredientCards = [
  {
    title: 'Konjac root foundation',
    body: 'A plant-based base that supports the shirataki rice identity.'
  },
  {
    title: 'Rice-style texture',
    body: 'A familiar grain-like experience for bowls, soups, stir-fries, and seasoned dishes.'
  },
  {
    title: 'Fast pantry use',
    body: 'An instant format designed for quick meals without losing a clean product story.'
  }
];
...
function ShelfToPlateExperience({ navigate }) {
  const [reveal, setReveal] = useState(56);
  const [language, setLanguage] = useState('bilingual');
  const [activeHotspot, setActiveHotspot] = useState(packageHotspots[0]);
  const [packageMode, setPackageMode] = useState('front');
  const [activeBlueprint, setActiveBlueprint] = useState(blueprintPanels[0]);
  const copy = languageModes[language];

  const updateRevealFromPointer = (event) => {
    if (event.pointerType && event.pointerType !== 'mouse') return;
    const bounds = event.currentTarget.getBoundingClientRect();
    const nextReveal = ((event.clientX - bounds.left) / bounds.width) * 100;
    setReveal(Math.min(86, Math.max(14, nextReveal)));
  };

  return (
    <section className="shelf-experience" aria-labelledby="shelf-experience-title">
      <div className="shelf-copy">
        <p className="eyebrow"><Sparkles size={16} /> Real product. Retail-ready presentation.</p>
        <h2 id="shelf-experience-title">From packed cartons to a polished pantry staple.</h2>
        <p>
          EveGlo Instant Shirataki Rice turns real packaged product into a cleaner shelf story for Canadian shoppers, retail buyers, and everyday low-carb meals.
        </p>
        <div className="language-switch" aria-label="Product language mode">
          {Object.entries(languageModes).map(([mode, item]) => (
            <button
              key={mode}
              type="button"
        ...
        <div className="shelf-actions">
          ...
          <button type="button" className="secondary-button" onClick={() => navigate('home', 'wholesale')}>
            See package details
          </button>
        </div>
      </div>

      <div className="retail-reveal" onPointerMove={updateRevealFromPointer}>
        ...
        <img className="reveal-before" src={experienceAssets.carton} alt="EveGlo product cartons packed for launch" />
        <img
          className="reveal-after"
          src={experienceAssets.polished}
          alt="Polished EveGlo Instant Shirataki Rice product presentation"
          style={{ clipPath: `polygon(0 0, ${reveal}% 0, calc(${reveal}% - 12%) 100%, 0 100%)` }}
        />
        <div className="reveal-sheen" style={{ left: `${reveal}%` }} />
        <label className="reveal-control">
          <span>Reveal product story</span>
          <input
            type="range"
            min="14"
            max="86"
            ...
            aria-label="Reveal polished product image"
          />
        </label>
      </div>
      ...
      <div className="package-story">
        <div>
          <div className="package-mode-switch" aria-label="Package view mode">
            <button
              type="button"
              className={packageMode === 'front' ? 'active' : ''}
              onClick={() => setPackageMode('front')}
            >
              Front details
            </button>
            <button
              type="button"
              className={packageMode === 'blueprint' ? 'active' : ''}
              onClick={() => setPackageMode('blueprint')}
            >
              Blueprint mode
            </button>
          </div>
          <div className={`package-stage ${packageMode === 'blueprint' ? 'blueprint-active' : ''}`}>
            {packageMode === 'front' ? (
              <>
                <img src={experienceAssets.front} alt="EveGlo Instant Shirataki Rice front package panel" />
                {packageHotspots.map((hotspot) => (
                  <button
                    type="button"
                    key={hotspot.id}
                    className={`hotspot-pin ${activeHotspot.id === hotspot.id ? 'active' : ''}`}
              ...
              <>
                <img src={experienceAssets.blueprint} alt="EveGlo carton dieline blueprint" />
                {blueprintPanels.map((panel) => (
                  <button
                    type="button"
                    key={panel.id}
                    className={`blueprint-pin ${activeBlueprint.id === panel.id ? 'active' : ''}`}
                    style={{ left: `${panel.x}%`, top: `${panel.y}%` }}
                    onClick={() => setActiveBlueprint(panel)}
                    onFocus={() => setActiveBlueprint(panel)}
                  >
                    {panel.label}
                  </button>
                ))}
        ...
        <article className="hotspot-panel">
          <p className="eyebrow">{packageMode === 'front' ? 'Package detail' : 'Blueprint mode'}</p>
          <h3>{packageMode === 'front' ? activeHotspot.title : activeBlueprint.label}</h3>
          <p>{packageMode === 'front' ? activeHotspot.body : activeBlueprint.note}</p>
          {packageMode === 'blueprint' && (
            <p className="blueprint-note">
              The dieline view shows the carton as a retail package system, with shopper-facing claims, operational areas, and side-panel information planned before shelf placement.
            </p>
          )}
        </article>
      </div>

      <div className="ingredient-theater">
        ...
        <article className="scan-card">
          <img src={experienceAssets.back} alt="EveGlo package back panel with product information" />
          <div>
            <h3>The package continues online.</h3>
            <p>Use the digital bridge for recipes, preparation videos, retailer information, and product updates connected to the physical package experience.</p>
          </div>
        </article>
      </div>
    </section>
      ...
      <main>
        {page === 'home' && (
          <>
            ...
            <ShelfToPlateExperience navigate={navigate} />
